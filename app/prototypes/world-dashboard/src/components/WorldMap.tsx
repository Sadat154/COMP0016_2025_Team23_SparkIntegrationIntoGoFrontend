...
type CountryMeta = { iso3: string; name?: string; society_name?: string; url_ifrc?: string };
...
export default function WorldMap({
  ...
  const [hover, setHover] = useState<{ iso3: string; name: string; x: number; y: number } | null>(null);
    ...
    Promise.all([
      ...
      .then(([world, countriesData]) => {
        ...
        for (const c of results) {
          if (c.iso3) map[c.iso3] = { iso3: c.iso3, name: c.name, society_name: c.society_name, url_ifrc: c.url_ifrc };
          if (c.name && c.iso3) nameToIso[String(c.name).trim().toLowerCase()] = c.iso3;
        }
  ...
  useEffect(() => {
    if (worldRef.current) draw(worldRef.current, countriesMeta);
  }, [highlightedIso3, width, height]);
  ...
  function draw(world: WorldGeoJSON, meta: Record<string, CountryMeta>) {
    ...
    svg
      ...
      .attr("fill", (d: any) => {
        ...
        if (!iso3 || iso3 === "-99") {
          const pname = String(d.properties?.["name"] ?? "").trim().toLowerCase();
      ...
      .on("mousemove", function (event: any, d: any) {
        ...
        const name = meta[iso3]?.name ?? String(d.properties?.["name"] ?? iso3);
        const [x, y] = d3.pointer(event, svgRef.current);
        setHover({ iso3, name, x: x + 12, y: y + 12 });
      })
  ...
  return (
    <div style={{ position: "relative", width, height }}>
      ...
      {hover && (
        ...
        >
          <div style={{ fontWeight: 700 }}>{hover.name} ({hover.iso3})</div>
          {hoverMeta?.society_name ? <div style={{ marginTop: 4 }}>{hoverMeta.society_name}</div> : null}
        </div>
      )}
    </div>
  );
